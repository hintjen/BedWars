...
| `runServer` crashes with `Failed to bind to port` / `Address already in use: bind` on port 25565, even though nothing looks like it's using it | A stale Windows port-forwarding rule (often left behind by an old WSL Minecraft server) is silently claiming the port. Check with `netsh interface portproxy show all`; if you see a rule listening on `25565`, remove it from an **elevated** (Run as Administrator) PowerShell: `netsh interface portproxy delete v4tov4 listenport=25565 listenaddress=0.0.0.0`. `netstat -ano \| findstr 25565` will show the port held by `svchost.exe` in this case — that's the giveaway (a real Java process would show up as `java.exe`, not `svchost.exe`). |
